...
def get_access_token() -> Optional[str]:
    ...
    if not MPESA_CONSUMER_KEY or not MPESA_CONSUMER_SECRET:
        logger.error("MPESA consumer credentials not set")
        return None
    ...
    try:
        ...
        logger.info("MPESA access token retrieved")
...
def initiate_stk_push(phone_number: str, amount: int) -> Dict:
    ...
    if not all([MPESA_SHORTCODE, MPESA_PASSKEY, MPESA_CALLBACK_URL]):
        return {"success": False, "message": "MPESA configuration incomplete"}
...
if __name__ == "__main__":
    result = initiate_stk_push("0714213413", 100)
    print(result)
